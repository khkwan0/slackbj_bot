...
if (opts.user) {
  ...
  walletTrans.forEach((trans, idx) => {
    ...
    if (now.isSameOrAfter(start) && now.isBefore(end)) {
      ...
      csv += `"${transTime.format(timestampFormat)}","${trans.type.toUpperCase()}",${trans.amount},${(trans.amount * GetPrice(trans.ticker, transTime.format('MMM DD, YYYY')))},"${trans.ticker}",${currPrice}${RenderRowBalances(walletValues, currencies, trans, now, zero)}`

      // handle fee
      if (typeof trans.fee !== 'undefined') {
        walletValues[trans.ticker] -= trans.fee
        csv += `"${moment.tz(parseInt(trans.timestamp)*1000, 'utc').format(timestampFormat)}","${trans.type.toUpperCase()} - FEE",${trans.fee},${trans.fee * GetPrice(trans.ticker, transTime.format('MMM DD, YYYY'))},"${trans.ticker}",${currPrice}${RenderRowBalances(walletValues, currencies, trans, now, zero)}`
      }
    }
  })
  console.log(csv)
